...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph" stylename="Headline 60pt"/>
  </xsl:variable>
  
  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="white" stylename="white text"/>
  </xsl:variable>
  ...
  <xsl:variable name="content">
    ...
    <content>
      <paragraph><white><xsl:value-of select="$contentXml/article/content/title"/></white></paragraph>
    </content>
  </xsl:variable>
