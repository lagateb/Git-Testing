...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    ...
    <map:entry element="cell_left" stylename="p_Tabelle_links"/>
    <map:entry element="cell_right" stylename="p_Tabelle_rechts"/>
  </xsl:variable>
  ...
  <xsl:template match="table">
    ...
    <xsl:variable name="colWidth" select="(187.08661417322833, 104.88188976377958)" as="xs:double*"/>
  ...
  <xsl:template match="cell">
    ...
    <xsl:call-template name="icml-tablecell">
      ...
      <xsl:with-param name="add-attributes" as="attribute()*">
        ...
        <xsl:attribute name="TextLeftInset" select="if ($isFirstCell) then 32.59842519685039 else 0"/>
        ...
        <xsl:attribute name="TextRightInset" select="if ($isFirstRow and $isFirstCell) then 14.173228346456694 else if ($colspan = 2) then 24.094488188976378 else if ($isFirstCell) then 5.669291338582678 else 24.094488188976378"/>
        <xsl:attribute name="LeftInset" select="if ($isFirstCell) then 32.59842519685039 else 0"/>
        <xsl:attribute name="TopInset" select="if ($isFirstRow) then 14.173228346456694 else 4.251968503937008"/>
        <xsl:attribute name="RightInset" select="if ($isFirstRow and $isFirstCell) then 14.173228346456694 else if(@colspan=2) then 24.094488188976378  else if ($isFirstCell) then 5.669291338582678 else 24.094488188976378"/>
        ...
        <xsl:attribute name="BottomEdgeStrokePriority" select="if ($isFirstRow) then 161 else 241"/>
        ...
        <xsl:attribute name="RightEdgeStrokePriority" select="if ($isFirstRow) then 145 else 241"/>
        <xsl:attribute name="LeftEdgeStrokePriority" select="if ($isFirstRow) then 141 else 241"/>
        ...
        <xsl:attribute name="BottomEdgeStrokeColor" select="if ($isFirstRow) then 'Color/C=35 M=4 Y=10 K=0' else 'Swatch/None'"/>
        <xsl:attribute name="TopEdgeStrokeColor" select="if ($isFirstRow) then 'Color/C=0 M=0 Y=0 K=88' else if ($isSecondRow) then 'Color/C=35 M=4 Y=10 K=0' else 'Swatch/None'"/>
        <xsl:attribute name="LeftEdgeStrokeColor" select="if ($isFirstRow) then 'Color/C=0 M=0 Y=0 K=88' else 'Swatch/None'"/>
        <xsl:attribute name="RightEdgeStrokeColor" select="'Swatch/None'"/>
        <xsl:attribute name="LeftEdgeStrokeColor" select="'Swatch/None'"/>
        <xsl:attribute name="BottomEdgeStrokeWeight" select="if ($isFirstRow) then 0.3 else 0"/>
        <xsl:attribute name="TopEdgeStrokeWeight" select="if ($isSecondRow) then 0.3 else 0"/>
  ...
  <xsl:template match="row" mode="icml-content">
    ...
    <xsl:if test="$isFirstRow">
      <xsl:copy>
        <cell colspan="2">
          <cell_headline>
            <cell_headline_style><xsl:apply-templates select="$masterStorageXml/article/content/headline" mode="icml-content"/></cell_headline_style>
          </cell_headline>
        </cell>
      </xsl:copy>
    </xsl:if>
