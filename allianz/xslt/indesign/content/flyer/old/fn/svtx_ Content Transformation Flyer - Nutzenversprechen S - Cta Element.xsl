...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph" stylename="Nutzenversprechen_CTA"/>
  </xsl:variable>

  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="paragraph_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'CTA_weiß')}"/>
  </xsl:variable>
  ...
  <xsl:template match="paragraph_style">
    ...
    <xsl:template match="footnote" mode="icml-content" >
        <xsl:copy-of select="svtx:getFootnoteNumber($contentToUseXml,.)"/>
    </xsl:template>
    <!-- Content -->
  <xsl:variable name="content">
    <content>
      <paragraph>
        <paragraph_style>
          <xsl:value-of select="$contentXml/article/content/calltoaction"/>
        </paragraph_style>
      </paragraph>
    </content>
  </xsl:variable>
