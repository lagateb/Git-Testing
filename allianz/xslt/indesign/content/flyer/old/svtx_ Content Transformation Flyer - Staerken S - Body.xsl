...
<xsl:stylesheet
  ...
  <xsl:variable name="CSMapping">
    <map:entry element="inline" stylename="{svtx:getCSMappingStyle($rootAsset, 'Copy_grau')}"/>
  ...
  <xsl:template match="inline | bold | sub | sup">
  ...
  <xsl:template match="bold" mode="icml-content">
    ...
    <paragraph><inline><xsl:copy-of select="node()"/></inline></paragraph>
  </xsl:template>
