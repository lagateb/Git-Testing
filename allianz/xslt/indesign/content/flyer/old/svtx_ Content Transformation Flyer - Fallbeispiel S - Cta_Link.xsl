...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph" stylename="Fallbeispiel_CTA"/>
  </xsl:variable>

  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="paragraph_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Copy_grau')}"/>
    <map:entry element="italic" stylename="{svtx:getCSMappingStyle($rootAsset, 'Copy_grau_italic')}"/>
  ...
  <xsl:template match="paragraph_style | italic | bold | sub ">
  ...
  <xsl:variable name="content">
    ...
    <content>
      <xsl:apply-templates select="$contentXml/article/content/calltoaction-link" mode="icml-content"/>
    </content>
  </xsl:variable>
  ...
  <xsl:template match="calltoaction-link" mode="icml-content">
    <paragraph>
      ...
      <xsl:if test="@url">
        <xsl:text>&#xD;</xsl:text><italic><xsl:value-of select="@url"/></italic>
      </xsl:if>
    </paragraph>
  </xsl:template>
