...
<xsl:stylesheet
    ...
    <!-- targetAssetId 16523
        <xsl:variable name="targetAssetId" select="16523"/>
    -->
        <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
    <!-- basic configuration          -->
    <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
    ...
    <xsl:variable name="contentXml" >
        <xsl:copy-of select="svtx:getXmlDataFromLayout($targetAssetId)"/>
    </xsl:variable>
    ...
    <xsl:variable name="content">
        <!--
        <xsl:variable name="extraFN"  select="svtx:getXmlDataFromLayout($targetAssetId)" />
        <xsl:message>FN=====<xsl:copy-of select="$extraFN"/>====</xsl:message>
        -->
        <xsl:message>FN1=====<xsl:copy-of select="$groupedFootnotes"/>====</xsl:message>
        <xsl:variable name="tmpData">
        <content>
            <footnotes>
        ...
        <xsl:message>FN2=====<xsl:copy-of select="$tmpData"/></xsl:message>
        <xsl:copy-of select="$tmpData"/>
    </xsl:variable>
    ...
    <xsl:template match="footnote" mode="icml-content">
        <!-- <xsl:copy><footnotes_style_sup><xsl:value-of select="./@no"/></footnotes_style_sup><xsl:value-of select="concat(normalize-space(text()),' ')"/></xsl:copy>
          Das ist bie PPTX <sup><xsl:value-of select="./@no"/></sup><xsl:copy-of select="."/>&#xA0;
        -->
