...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph-text" stylename="Copy_10/13.25_ASL_A4mag"/>
    <map:entry element="intro-text" stylename="Copy_10/13.25_ASL_A4mag"/>
  </xsl:variable>
  ...
  <xsl:template match="paragraph-text | intro-text | bullet-list-item">
  ...
  <xsl:variable name="content">
    ...
    <content>
      <xsl:apply-templates select="$contentXml/article/content/text"/>
    </content>
  </xsl:variable>
  ...
  <xsl:template match="intro">
    <intro-text><xsl:copy-of select="node()"/></intro-text>
  </xsl:template>
  
  <xsl:template match="paragraph">
    <paragraph-text><xsl:copy-of select="node()"/></paragraph-text>
  </xsl:template>
