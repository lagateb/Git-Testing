...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph" stylename="Headline 24pt"/>
  </xsl:variable>
  
  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="yellow" stylename="energetic yellow"/>
  </xsl:variable>
  ...
  <xsl:variable name="content">
    ...
    <content>
      <paragraph><yellow><xsl:value-of select="$contentXml/article/content/subtitle"/></yellow></paragraph>
    </content>
  </xsl:variable>
