...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="productprovider" stylename="p_DSN_Copyright"/>
  </xsl:variable>

  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="productprovider_style" stylename="{svtx:getCSMappingStyleCD21($rootAsset, '')}"/>
  </xsl:variable>
  ...
  <xsl:template match="productprovider" mode="icml-content">
    <productprovider><productprovider_style><xsl:apply-templates/></productprovider_style></productprovider>
  </xsl:template>
