...
<xsl:stylesheet
  ...
  <xsl:template match="sup|sub" mode="icml-content">
    ...
    <xsl:template match="text()" mode="onlyText">
        <xsl:variable name="cleanerText" select="replace(replace(., $PPTX_LINE_BREAK, ''), $FLYER_LINE_BREAK, '')"/>
        <xsl:value-of select="$cleanerText"/>
    </xsl:template>
