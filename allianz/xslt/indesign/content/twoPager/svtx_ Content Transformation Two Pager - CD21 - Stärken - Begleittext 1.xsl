...
<xsl:stylesheet
    ...
    <xsl:variable name="CSMapping">
        <map:entry element="paragraph_style" stylename="{svtx:getCSMappingStyle($rootAsset, '')}"/>
        <map:entry element="headline_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'highlight')}"/>
        <map:entry element="sup" stylename="{svtx:getCSMappingStyle($rootAsset, 'hochgestellt')}"/>
    </xsl:variable>
