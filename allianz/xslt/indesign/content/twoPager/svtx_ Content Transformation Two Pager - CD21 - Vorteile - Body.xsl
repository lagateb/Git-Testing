...
<xsl:stylesheet
  ...
  <xsl:template match="headline" mode="icml-content">
    <xsl:variable name="txt">
      <xsl:apply-templates select="* except footnote" mode="onlyText"/>
    </xsl:variable>
    <xsl:if test="$txt">
      <bold>
        <xsl:value-of select="$txt"/><xsl:apply-templates select="footnote" mode="icml-content"/><xsl:text>: </xsl:text>
      </bold>
    </xsl:if>
  </xsl:template>
