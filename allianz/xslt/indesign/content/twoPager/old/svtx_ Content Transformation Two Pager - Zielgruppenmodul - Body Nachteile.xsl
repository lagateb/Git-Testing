...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="paragraph" stylename="Zielgruppe_Body_negativ"/>
  </xsl:variable>
  ...
  <xsl:variable name="CSMapping">
    ...
    <map:entry element="sup" stylename="{svtx:getCSMappingStyle($rootAsset, 'Copy_schwarz_hochgestellt')}"/>
  ...
  <xsl:variable name="content">
    ...
    <content>
      <xsl:apply-templates select="$contentXml/article/content/body/contra/bullet-list" mode="icml-content"/>
    </content>
  </xsl:variable>
