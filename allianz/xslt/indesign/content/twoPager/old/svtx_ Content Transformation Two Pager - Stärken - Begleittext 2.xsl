...
<xsl:stylesheet
    ...
    <xsl:variable name="PSMapping">
        <map:entry element="paragraph" stylename="StärkenAllianz_Begleittext2"/>
        <map:entry element="headline" stylename="StärkenAllianz_Begleittext2_head"/>
    </xsl:variable>
    ...
    <xsl:variable name="content">
        ...
        <content>
            <xsl:apply-templates select="$contentXml/article/content/seals/seal[2]" mode="icml-content"/>
        </content>
    </xsl:variable>

    <xsl:template match="seal[2]" mode="icml-content">
