...
<xsl:stylesheet
  ...
  <xsl:variable name="CSMapping">
    ...
    <map:entry element="bullet_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Copy_grau')}"/>
    <map:entry element="bold" stylename="Copy_grau_Highlight"/>
  ...
  <xsl:template match="paragraph_style | bullet_style | bold | sup | sub">
