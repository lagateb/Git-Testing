...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    ...
    <map:entry element="paragraph" stylename="Produktbeschreibung_Body"/>
    <map:entry element="headline" stylename="Produktbeschreibung_Headline"/>
  </xsl:variable>
  ...
  <xsl:variable name="CSMapping">
    ...
    <map:entry element="headline_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Headline_Versal_grau')}"/>
  ...
  <xsl:template match="paragraph | bullet_list | headline">
    <xsl:call-template name="icml-paragraph"/>
  </xsl:template>

  <!-- Inline-Elemente, können bei Bedarf Zeichenformat erhalten -->
  <xsl:template match="paragraph_style | bold | sup | headline_style | sub">
  ...
  <!-- Fussnoten extrahieren für Seite-->
  ...
  <xsl:variable name="content">
    ...
    <content>
      <xsl:apply-templates select="$transformedContentXml/article/content/headline" mode="icml-content"/>
