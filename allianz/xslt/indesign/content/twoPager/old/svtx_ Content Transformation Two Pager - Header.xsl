...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="headline" stylename="Titel_Headline"/>
    <map:entry element="subline" stylename="Titel_Subline"/>
  </xsl:variable>

  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="headline_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Headline_Titel_grau')}"/>
    <map:entry element="subline_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Subline_Titel_grau')}"/>
  </xsl:variable>
  ...
  <xsl:template match="text()" mode="icml-content">
    ...
    <xsl:for-each select="$tokenized">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="if (position() ne last()) then replace(concat(normalize-space(.), '-'), '--', '-') else normalize-space(.)"/>
        </xsl:otherwise>
      </xsl:choose>
