...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="headline" stylename="Produktdetails_Headline"/>
  </xsl:variable>
  
  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="headline_style" stylename="{svtx:getCSMappingStyle($rootAsset, 'Headline_Versal_schwarz')}"/>
  </xsl:variable>
