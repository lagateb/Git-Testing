<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="content">
    <div class="c-chapter__content" style="top: 0px; left: 0px; transform: scale({$scale},{$scale}); -ms-transform: scale({$scale},{$scale}); -moz-transform: scale({$scale},{$scale}); -webkit-transform: scale({$scale},{$scale});">
            ...
            <div class="l-grid__row">
              <div class="l-grid__column-4 u-hidden-small-down">
                <div class="c-text-and-image__image-column c-text-and-image__image-column--default">
                  <div class="c-text-and-image__image-wrapper js-default-component" id="c-text-and-image820772259" data-is-init="true">
  ...
  <xsl:template match="bullet-list">
    <xsl:for-each select="item/paragraph">
      <li class="c-list__item"><xsl:apply-templates/></li>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:function name="svtx:getImageElementOfHref" as="element(img)?">
    ...
    <xsl:if test="$assetID">
      ...
      <img src="{concat($urlPrefix, 'assets/asset/id/', $storageItem/@asset_id, '/storage/', $storageKey, '/file/', tokenize($storageItem/@relpath,'/')[last()])}" alt="{$asset/@name}" title="{$asset/@name}" class="c-image__img" sizes="(max-width: 1110px) 100vw, 1110px"/>
    </xsl:if>
  </xsl:function>
</xsl:stylesheet>
