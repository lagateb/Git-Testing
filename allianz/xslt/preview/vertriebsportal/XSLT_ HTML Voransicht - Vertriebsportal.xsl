<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="previewMapping">
     <entry type="article.header." key="svtx:html-preview.vertriebsportal.header"/>
    <entry type="article.vorteile." key="svtx:html-preview.vertriebsportal.vorteile"/>
    <entry type="article.produktbeschreibung." key="svtx:html-preview.vertriebsportal.produktbeschreibung"/>
    <entry type="article.funktionsgrafik." key="svtx:html-preview.vertriebsportal.funktionsgrafik-quer"/>
    <!-- <entry type="article.nutzenversprechen." key="svtx:html-preview.vertriebsportal.nutzenversprechen"/> -->
    <entry type="article.productdetails." key="svtx:html-preview.vertriebsportal.produktdetails"/>
    <entry type="article.zielgruppenmodul." key="svtx:html-preview.vertriebsportal.zielgruppen-modul"/>ar
  </xsl:variable>
    ...
    <xsl:template match="/asset">
        <html>
            <head>
                ...
                <xsl:variable name="cssAsset" select="cs:get-resource-asset('svtx:cs5.article-text-preview-vertriebsportal.css')"/>
                ...
                <xsl:variable name="jsAsset" select="cs:get-resource-asset('svtx:cs5.article-text-preview-vertriebsportal.js')"/>
                <xsl:if test="$jsAsset">
                    <script src="{concat('/censhare5/client/rest/service/assets/asset/id/', $jsAsset/@id, '/version/', $jsAsset/@version, '/storage/master/file/csTextAssetPreview.js')}"></script>
                </xsl:if>
