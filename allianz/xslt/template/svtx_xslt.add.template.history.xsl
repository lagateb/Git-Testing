<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/" priority="10">
    ...
    <xsl:message>
      <xsl:text>Updated </xsl:text> <xsl:value-of select="$mediaAsset/@name"/> with <xsl:value-of select="$note"/> or <xsl:copy-of select="$data"/>
    </xsl:message>

    <!-- /asset_feature[@feature='censhare:resource-key']/@value_string" -->

    <xsl:copy-of select="svtx:add-change-history($mediaAsset, $note)"/>
