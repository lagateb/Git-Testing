...
<xsl:stylesheet
  ...
  <xsl:template match="paragraph | bullet-list | headline">
  ...
  <xsl:variable name="content">
    <content>
      ...
      <content>
        <xsl:apply-templates select="$contentXml/article/content/seals/seal[1]" mode="icml"/>
      </content>
    </content>
  </xsl:variable>

  <xsl:template match="seal" mode="icml">
    <xsl:apply-templates mode="icml"/>
  </xsl:template>

  <xsl:template match="headline" mode="icml">
    <xsl:copy><bold><xsl:apply-templates mode="icml"/></bold></xsl:copy>
  </xsl:template>
