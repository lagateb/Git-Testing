...
<xsl:stylesheet
    ...
    <!-- baut aus einer struktur headline mit body1 zu einem Body um -->
    <!-- Flex Tile  ORTCONTENT-608 -->
    <!-- wird dann in der PPTX wie folgt eingebunden svtxtr:svtx:pptx.transform_merge_headline_body[x]
     x => 1..
    -->
        ...
        <xsl:if test="$storageContent">
            ...
            <xsl:if test="$data">
                <body><xsl:copy-of select="$data/headline/."/>
                    <xsl:if test="$data/body1/*[1]/local-name()='paragraph'"><break/></xsl:if>
                    <xsl:copy-of select="$data/body1/."/>
                </body>
