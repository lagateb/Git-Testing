...
<xsl:stylesheet
  ...
  <!-- call to action auslesen und umformen für    svtx:content-editor-free-modules -->
  ...
  <xsl:template match="asset[starts-with(@type, 'text.')]">
    ...
    <xsl:if test="$storageConent">
      <xsl:variable name="cta" select="$storageConent/article/content/body/bullet-list/item[$transParamInt]/calltoaction-link" as="element(calltoaction-link)?"/>
