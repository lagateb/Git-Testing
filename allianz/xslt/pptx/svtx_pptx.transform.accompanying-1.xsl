...
<xsl:stylesheet
  ...
  <xsl:variable name="debug" select="true()" as="xs:boolean"/>
    ...
    <xsl:template match="asset[starts-with(@type, 'text.')]">
        ...
        <xsl:if test="$storageContent"><accompanying-1><headline><color value="#113388"><font-size value="10.5"><bold><xsl:apply-templates
                select="$content/article/content/accompanying-1/headline"/></bold></font-size></color></headline>
            <paragraph><break/><color value="#000000"><font-size value="9.0"><xsl:apply-templates select="$content/article/content/accompanying-1/paragraph"/></font-size></color>
            </paragraph>  </accompanying-1>
