...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="headline" stylegroup="Stärken_Allianz" stylename="StärkenAllianz_Headline"/>
    <map:entry element="paragraph" stylegroup="Stärken_Allianz" stylename="StärkenAllianz_Body"/>
    <map:entry element="headline_accompanying" stylegroup="Stärken_Allianz" stylename="StärkenAllianz_Begleittext1_head"/>
    <map:entry element="paragraph_accompanying" stylegroup="Stärken_Allianz" stylename="StärkenAllianz_Begleittext1"/>
  </xsl:variable>

  <!-- Character mappings -->
  <xsl:variable name="CSMapping">
    <map:entry element="headline_style" stylegroup="Head+Subline" stylename="Headline_Versal_orange"/>
    <map:entry element="paragraph_style" stylegroup="Copy/Grau" stylename="Copy_grau"/>
    <map:entry element="headline_accompanying_style" stylegroup="Copy/Grau" stylename="Copy_mittel_grau"/>
    <map:entry element="paragraph_accompanying_style" stylegroup="Copy/Grau" stylename="Copy_klein_grau"/>
  ...
  <xsl:template match="headline | paragraph | headline_accompanying | paragraph_accompanying">
    <xsl:call-template name="icml-paragraph" />
  </xsl:template>

  <!-- Inline-Elemente, können bei Bedarf Zeichenformat erhalten -->
  <xsl:template match="headline_style | paragraph_style | bold | headline_accompanying_style | paragraph_accompanying_style | footnote_style">
  ...
  <xsl:variable name="content">
    ...
    <content>
      <xsl:choose>
        ...
        <xsl:when test="lower-case($placementPart) = 'accompanying'">
          <xsl:apply-templates select="$contentXml/article/content/accompanying-1[1]" mode="icml-accompanying"/>
        </xsl:when>
      </xsl:choose>
    </content>
  </xsl:variable>
  ...
  <xsl:template match="text()" mode="icml-body icml-headline icml-accompanying">
    <xsl:value-of select="."/>
  </xsl:template>
  ...
  <!-- accompanying part -->
  <xsl:template match="accompanying-1" mode="icml-accompanying">
    <xsl:apply-templates mode="icml-accompanying"/>
  </xsl:template>

  <xsl:template match="headline" mode="icml-accompanying">
    <headline_accompanying>
      <headline_accompanying_style>
        <xsl:apply-templates mode="icml-accompanying"/>
      </headline_accompanying_style>
    </headline_accompanying>
  </xsl:template>

  <xsl:template match="paragraph" mode="icml-accompanying">
    <paragraph_accompanying>
      <paragraph_accompanying_style>
        <xsl:apply-templates mode="icml-accompanying"/>
      </paragraph_accompanying_style>
    </paragraph_accompanying>
  </xsl:template>

  <xsl:template match="footnote" mode="icml-body icml-accompanying">
