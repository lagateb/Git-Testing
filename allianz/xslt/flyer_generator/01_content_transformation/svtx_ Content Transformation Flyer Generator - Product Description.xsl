...
<xsl:stylesheet
  ...
  <xsl:variable name="PSMapping">
    <map:entry element="headline" stylegroup="Produktbeschreibung" stylename="Produktbeschreibung_Headline"/>
    <map:entry element="paragraph" stylegroup="Produktbeschreibung" stylename="Produktbeschreibung_Body"/>
    <map:entry element="bullet-list" stylegroup="Produktbeschreibung" stylename="Produktbeschreibung_Body_Bullets"/>
  </xsl:variable>
  ...
  <xsl:template match="headline | paragraph | bullet-list">
    <xsl:call-template name="icml-paragraph" />
  </xsl:template>

  <!-- Inline-Elemente, können bei Bedarf Zeichenformat erhalten -->
  <xsl:template match="headline_style | paragraph_style | bold | footnote_style">
  ...
  <!-- -->
  <xsl:template match="item/paragraph" mode="icml-body">
    <xsl:apply-templates mode="icml-body"/><break/>
  </xsl:template>

  <!-- -->
  <xsl:template match="bold" mode="icml-body">
    <bold><xsl:apply-templates mode="icml-body"/></bold>
  </xsl:template>

  <!-- -->
  <xsl:template match="footnote"  mode="icml-body">
    <xsl:variable name="footNoteNumber" select="count(preceding::footnote) + 1" as="xs:long?"/>
