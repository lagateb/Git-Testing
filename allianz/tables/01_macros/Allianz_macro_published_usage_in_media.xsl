...
-->
  ...
  <xsl:function name="svtx:articleHasPSB" as="xs:boolean">
    ...
    <xsl:variable name="layout" select="($article/cs:child-rel()[@key='user.main-content.'])/cs:parent-rel()[@key='actual.']/cs:asset()[@censhare:asset.type='layout.*' and @svtx:layout-template-resource-key='svtx:indd.template.twopager.allianz']"/>
    <xsl:value-of select="if($layout[@wf_step=(30,60,55,90)]) then true() else false()"/>
  </xsl:function>


  <xsl:function name="svtx:articleHasFlyer" as="xs:boolean">
    <xsl:param name="article" as="element(asset)" />
    <xsl:variable name="layout" select="($article/cs:child-rel()[@key='user.main-content.'])/cs:parent-rel()[@key='actual.']/cs:asset()[@censhare:asset.type='layout.*' and @svtx:layout-template-resource-key='svtx:indd.template.flyer.allianz']"/>
    <xsl:value-of select="if($layout[@wf_step=(30,60,55,90)]) then true() else false()"/>
  </xsl:function>
  ...
  <xsl:function name="svtx:articleHasPPTX" as="xs:boolean">
    ...
    <xsl:variable name="issue" select="$pptx/cs:parent-rel()[@key='target.']/cs:asset()[@censhare:asset.type = 'presentation.issue.']" as="element(asset)*"/>
    <xsl:value-of select="if($issue[@wf_step=(30,60,55,90)]) then true() else false()"/>
  </xsl:function>
  ...
  <xsl:function name="svtx:getArticleChannels" as="xs:string">
    <xsl:param name="article" as="element(asset)" />
    <xsl:variable name="allChannels" select="($article/cs:child-rel()[@key='user.web-usage.']/cs:asset()[@censhare:asset.type = 'text.public.*'])/asset_feature[@feature='censhare:output-channel']/@value_key"/>
