...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/" >
        ...
        <xsl:variable name="result" select="count($data/data/retValue[@val='0'])=0"/>
    ...
    <xsl:function name="svtx:checkApprovedforUse">
        ...
        <retValue>
            <xsl:choose>
                <xsl:when test="$textAsset/@wf_step != '30' and  count($textAsset/asset_feature[@feature='svtx:preselected-output-channel'])>0">
